...
parser ingress_parser( packet_in P, out headers_t H,
  ...
  state parse_dpa {
    P.extract(H.dpa);
    /// Optimization: do quants only at one 0. Parse quants on first input, and then setValid only on last output
    ///
    /// Problem: If we parse quants only once at the start, the compiler seems to put them on a PHV slot shared with other fields.
    /// This is what we observe from the parser log:
    ///
    ///  'parse_ml':          extracted 0x00000001, updating PHV word   8 to value 0x00000001 I [H.dpa.job[31:0]]
    ///  'parse_ml':          extracted 0x00000001, updating PHV word   7 to value 0x00000001 I [H.dpa.seq[31:0]]
    ///   ...
    ///  'parse_ml.$split_0': extracted 0x00000002, updating PHV word   0 to value 0x00000002 I [H.arp_ip4.dst_proto_addr[31:0], H.dpa_quants.v00[31:0], M.dpa.bitmap_pre[31:0]]
    ///
    /// Due to this sharing and since bitmap_pre is computed before the exponent reduction, what ends up happening is that bitmap_pre
    /// is the actual value used for the exponent reduction instead of the real exponent. I.e. the exponent is parsed correctly, but is 
    /// overwritten by the dpa_bitmap table.
    ///
    /// This is either a compiler bug i.e. the compiler can't see dpa_quants and bitmap_pre should not share a PHV slot, or we don't handle 
    /// parsing/deparsing correctly somehow.
    /// Anyway, we can live with the "inefficiency" of parsing/deparsing 1-4 exponends at every pipe. But for future reference,
    /// what we want to see is this:
    ///
    ///  'parse_ml': extracted 0x00000001, updating PHV word   8 to value 0x00000001 I [H.dpa.job[31:0]]
    ///  'parse_ml': extracted 0x00000001, updating PHV word   7 to value 0x00000001 I [H.dpa.seq[31:0]]
    ///  'parse_ml': extracted 0x00000002, updating PHV word   1 to value 0x00000002 I [H.arp_ip4.src_proto_addr[31:0], H.dpa.bitmap[31:0]]
    ///  'parse_ml': extracted 0x00000002, updating PHV word  11 to value 0x00000002 I [H.dpa_quants.v00[31:0]]
    ///   ...
    ///
    /// TODO: Try handle quants still at pipe 0 but now at the very end of the (cross-pipe) traversal. I.e:
    ///   - At INPUT if 1/1 mode, 
    ///   - At OUTPUT_0 if N/1 mode, or 
    ///   - At OUTPUT_1 if N/2 mode
    transition parse_dpa_payload;
  }

  // state parse_dpa_quants {
  //   P.extract(H.dpa.quant);
  //   transition parse_dpa_values;
  // }
