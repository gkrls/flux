...
control dpa_value_reducer_single ( in ingress_metadata_t M, in dpa_slot_t idx, in dpa_value_t val, inout dpa_value_t res)
  ...
  table reducer_table {
    key = { M.dpa.kind: ternary;
            ...
            M.dpa.count: ternary;       }
    actions = { NoAction; read; write; update; }
    const size = 7;
...
control dpa_value_reducer_double ( in ingress_metadata_t M, in dpa_slot_t idx,
  ...
  table reducer_table {
    key = { M.dpa.kind : ternary;
            ...
            M.dpa.bitmap: ternary;
            M.dpa.count : ternary;       }
    actions = { NoAction; read_lo; read_hi; write; update; }
    const size = 8;
    const default_action = NoAction;
    const entries = {
...
control dpa_exponent_reducer( in headers_t H, in ingress_metadata_t M, in dpa_slot_t idx,
  ...
  table reducer_table {
    ...
    const entries = {
      ...
      // ( P_INPUT   , S_EXP,  _         , C_COMPLETE_EARLIER ) : read();
      ( P_INPUT   , S_NEW,  _         , _                  ) : write();
      ( P_INPUT   , S_OLD,  _         , _                  ) : read();   // read for implicit SYN
      ( P_SYN     , _    ,  _         , _                  ) : read();
      ( P_INPUT   , S_HI ,  _         , _                  ) : NoAction();
    }
  }
...
control dpa_reduce( inout headers_t H, inout ingress_metadata_t M) {
